...
export function StealBanner({ activePlayerName, failedCount, failureReason }: StealBannerProps) {
  return (
    <GlassPanel tone="tight" accent="secondary" className="rounded-[2rem] bg-[linear-gradient(135deg,rgba(0,229,255,0.18),rgba(45,27,66,0.94))] p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div>
          <p className="font-label text-[0.72rem] font-bold uppercase tracking-[0.22em] text-[var(--arcade-yellow)]">Steal activated</p>
          <p className="font-body text-sm text-on-surface">
            {activePlayerName} is up next. {failedCount} player{failedCount === 1 ? '' : 's'} already out on this question.
          </p>
        </div>
      </div>
    </GlassPanel>
